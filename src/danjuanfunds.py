    ...
    if funds:
        for fund in funds:
            ...
            if resp.status_code == 200:
                ...
                if items:
                    if items[0].get("date") == fund.get("last_update"):
                        print(f"基金 {fund.get('code')} 无更新, 最新净值日期: {items[0].get('date')}")
                    else:
                        print(f"基金 {fund.get('code')} 有更新, 最新净值日期: {items[0].get('date')}")
                        fund["last_update"] = items[0].get("date")
